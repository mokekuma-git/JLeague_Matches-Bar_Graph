"""Tests for the 2026 special season's grouped structure.
...
The tests that parsed that season's match pages were dropped when jleague.jp
was rebuilt as a client-rendered site and the reader moved to the J-League Data
Site (SFMS01), which publishes no group column.  What remains here covers the
season-map side of the grouped structure and how the fetched data is split
across sub-season CSVs, both of which are still live.
"""
...
class TestUpdateSubSeasonMatches(unittest.TestCase):
    ...
    @patch('read_jleague_matches.read_matches')
    @patch.object(mu, 'get_csv_path')
    def test_missing_group_column_raises(self, mock_csv_path, mock_read_range, mock_update):
        """Grouped sub-seasons cannot be split when the source has no group column."""
        mock_csv_path.side_effect = lambda comp, name=None: f'/tmp/{comp}_{name}.csv'
        # The Data Site returns no 'group' column.
        ungrouped = pd.DataFrame([dict(self.BASE_ROW, home_team='Home1', away_team='Away1')])
        mock_read_range.return_value = ungrouped
