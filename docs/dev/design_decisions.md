...
- **`csv_timestamp.csv` は「最終変更時刻」** — 最終取得時刻ではない。`update_timestamp()` は `update_csv()` からのみ呼ばれ、`update_if_diff()` が差分を認めたときにだけ到達する。取得しても内容が同じなら更新しない
  - 理由: 「最終取得」の意味にすると、取得のたびにこのファイルだけが変わり、中身の変わらないコミットと Pages デプロイが積み上がる (#316)
  - 影響: 差分更新の対象セクション選択 (`_get_sections_since()`) の起点が「最終変更」になるため、窓は従来より**広くなる方向にしかずれない**。取りこぼしは増えず、変化が無い間は同じセクションを取得し直す
- **定期実行の cron は「起床の合図」であって予約時刻ではない** — GitHub は schedule 起動を数時間遅らせることが常態化している (#315)。run が何をするかは起動時刻や発火した cron 行ではなく、データと状態から決める
  - ライブ監視 (`live-update-csv.yaml`): 監視対象日は CSV から決める (前日に開始済み・未終了の試合が残っていれば前日)。起床は2時間おきで、どの遅延でも試合中に監視 run が存在する。同時実行は concurrency で1本に制限し、予算切れ時は待機中の run が引き継ぐ
  - 日次フル更新 (`update-match-csv.yaml`): 「その JST 日のフル更新が済んだか」を Actions cache の記録 (キー `daily-update-<JST日付>`) で判定し、最初に着いた run が担う。予備スロットは済んでいれば即終了する。記録はフル更新が成功したときだけ書くので、失敗した日は次のスロットが再試行する。記録を引けない場合は「未実施」とみなす (取りこぼすより二重実行を選ぶ)
- **CSV の差分判定は「保存される文字列」で行う** (`_as_stored()`) — `DataFrame.equals()` は値だけでなく dtype も一致を要求する。CSV から読み戻した frame は文字列 dtype、取得した frame は試合の無い月の空 frame と concat された時点で object dtype になり、値が同じでも常に「差分あり」になる (#316)
