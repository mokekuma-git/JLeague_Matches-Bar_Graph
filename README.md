...
Jリーグについては、1993年の開幕以降すべての試合データを取得済みです。  
一時期に実施していた前期後期制の時期や、2026の秋春制移行期の百年構想リーグなどは複数グループに分けてそれぞれを1シーズンとしてグラフ化できます。  
仮想的な表示として、1年単位で全試合を同じリーグであると想定したグラフも作ったりしているので、ご興味があれば選択してみてください。(その年の年号のみをシーズンとして選択)  

各シーズンで採用される、PK勝ちやゴールデンゴールなどの特殊ルールも、少しずつ対応していく予定です。
...
Jリーグのデータは、Jリーグのサイトで公開される <https://www.jleague.jp/match/section/j1/1/> などから、その他ユースリーグなどはJFAの試合情報 <https://www.jfa.jp/match/takamado_jfa_u18_premier2025/east/match/schedule.json> などから取得しています。  
ACLのデータは、Yahoo スポーツナビさんの<https://soccer.yahoo.co.jp/jleague/category/acl/schedule/> から取得しています。  
設定しているのはこの設定ファイル [config/jleague.yaml](config/jleague.yaml) / [config/jfamatch.yaml](config/jfamatch.yaml)  
変換した結果はこちらです。[docs/csv/](docs/csv/)

各節の試合終了後 (あるいは日本時間深夜) にスクリプトで取得しています。  
...
- **表示ラウンドの選択** — 表示する開始ラウンドを選べます (例: ラウンド16から / 準々決勝から)
- **ズーム** — ブラケット全体の拡大・縮小ができます
- **H&A (2試合制)** — ルヴァンカップなどで採用される2試合合計方式に対応し、各試合のスコアはカードのホバーなどで確認できます
- **セクション分割** — 大規模な大会で途中に抽選などが入る場合、グループ別に折りたたみ表示する設定も作っています
...
最新のものを確認したい場合はご自身でこのプロジェクトを clone して、以下のスクリプトを実行してください (uvから pyproject.toml で示すライブラリを準備してpythonを呼ぶ方法を推奨します)  
